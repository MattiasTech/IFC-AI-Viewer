...
console.info("🚀 App entry loaded (app.js)");
...
async function onAsk() {
  ...
  if (!apiKey) {
    const pass = passphraseInput.value.trim();
    try { apiKey = await loadSecret('openai_key', pass || null); } catch { apiKey = ''; }
  }
  ...
  try {
    ...
    const spec = await planFilterWithLLM({ apiKey, model: 'gpt-4o-mini', userPrompt, index: viewer.index, strict: strictChk.checked });
